...
class plgFinderHikashop extends plgFinderHikashopBridge
{
	...
	public function _onFinderAfterSave($context, $row, $isNew)
	{
		if ($context == 'com_hikashop.product' && !is_null($row))
		{
			...
			$index_variants = $this->params->get('index_variants', 1);
			if(!$index_variants) {
				if((isset($row->product_type) && $row->product_type == 'variant') || (isset($row->old) && isset($row->old->product_type) && $row->old->product_type == 'variant')) {
					return true;
				}
			}
