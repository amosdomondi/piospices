...
class plgHikashopshippingFedEx2 extends hikashopShippingPlugin
{
	var $pluginConfig = array(
		...
		'special_service_types' => array('SPECIAL_SERVICE_TYPES', 'checkbox', array(
	...
	protected function _fedex_request_rates ($method_params, $address, $order) {
		...
		if(!empty($address->shipping_address->address_company)) {
			$array_address['address']->setResidential(false);
		} else {
			$array_address['address']->setResidential(true);
		}
		try {
			...
			if (isset($auth_result->access_token)) {
				...
				if(!empty($method_params->shipping_params->special_service_types)) {
					$specialServices = new ShipmentSpecialServices();
					$specialServices->setSpecialServiceTypes($method_params->shipping_params->special_service_types);
					$createRatesRequest->setShipmentSpecialServices($specialServices);
				}
				$weight_unit = 'kg';
